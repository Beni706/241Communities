...
/// Lecture (GET)
///   (GET) http://localhost:3000/api/formateur

const prisma = new PrismaClient();

// Cette fonction est appelée pour toute requête GET sur /api/formateur
export async function GET(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to fetch formateur" }, { status: 500 })
  }
}
...
// Création (POST)
// (POST) http://localhost:3000/api/formateur
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!nom || !prenom || !email || !password || !referentiel) {
    ...
    return NextResponse.json({message: 'Formateur ajouté avec succès', newFormateur}, { status: 201 });
