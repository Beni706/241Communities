...
export default function ApprenantDetailPage() {
  ...
  useEffect(() => {
    const fetchApprenant = async () => {
      try {
        ...
        // 1. Récupération des détails de l'apprenant
        ...
        if (!apprenantResponse.ok) {
          throw new Error("Failed to fetch apprenant")
        }

        const apprenantData = await apprenantResponse.json()
        setApprenant(apprenantData)

        // 2. Récupération des cours (filtrés par référentiel)
        ...
        if (coursResponse.ok) {
          ...
          setCours(coursData.filter((cours: Cours) => cours.referentiel === apprenantData.referentiel))
        }

        // 3. Récupération des veilles (filtrées par référentiel)
        ...
        if (veillesResponse.ok) {
          ...
          setVeilles(veillesData.filter((veille: Veille) => veille.referentiel === apprenantData.referentiel))
        }

        // 4. Récupération des soumissions de l'apprenant
        ...
        console.error("Error fetching apprenant data:", error)
  ...
  // Vérifie si une veille est encore active
  ...
  // Contenu de la page (extrait du layout pour éviter la duplication)
  const renderContent = () => {
    if (!apprenant) {
      return (
          <Card>
