...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = async (
    ...
    try {
      ...
      if (!data.token || !data.id) {
        throw new Error(
          "Réponse de l'API de connexion invalide: token ou ID manquant."
        );
      }
      ...
      if (role === "formateur") {
        try {
          const userResponse = await fetch(
            `${API_BASE_URL}/formateur/${data.id}`,
            {
  ...
  const logout = () => {
    ...
    localStorage.removeItem("user");
    setUser(null);
    // Modifier cette ligne pour rediriger vers la page d'accueil au lieu de /login
